...
// frequency-table of tree distance, O(nlog^2n)
// @param  adj
// @return freq 'freq[i] := #(u, v) s.t. u < v and dist(u, v) = i'
auto get_freq = [](const auto& adj) {
	...
	auto update = [&](const auto& buc) {
		...
		for (const auto& d : buc) {
			...
			while (acc.size() < d.size()) acc.push_back(0);
